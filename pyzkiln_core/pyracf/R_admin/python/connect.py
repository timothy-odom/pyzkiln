...
class Connect:
    def __init__(self, racf=None, radmin=None, func_type=None):
        ...
        self.racf.log.debug('Connect init')
    ...
    def run(self, rafcomm,prof_name,parms):
        self.racf.log.debug('Connect run')
        proc = subprocess.run(['./CONNECT.rexx',rafcomm, prof_name,parms],cwd=PY_REXX+'/R_admin/python/REXX',capture_output=True, text = False)
        return proc
