...
class Rdel:
    ...
    def run(self,prof_name,params):
        self.racf.log.debug('RDEL run')

        proc = subprocess.run(['./RDEL.REXX',prof_name,params],cwd=PY_REXX+'/R_admin/python/REXX',capture_output=True,text=False)
        return proc
